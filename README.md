...
Otherwise with `requirements.txt`
```
conda create --name <name-env> --file requirements.txt
...
4. Verify GPU support
    ...
    For Windows some libraries TensorFlow are deprecated:
    If the code didn't detect the GPU please install TensorFlow with Cuda support: (code from https://www.tensorflow.org/install/pip?hl=it#windows-native)
    ```
    ...
    #### For Linux
    If the code didn't detect the GPU install TensorFlow with Cuda https://www.tensorflow.org/install/pip?hl=it#linux
...
For the MiniRocket there is one single file `MiniRocket.py`.
